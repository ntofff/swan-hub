...
const ReportFolderManager = ({ folders, colorOptions, onClose }: Props) => {
  ...
  return (
    ...
    >
      <div
        className="w-full sm:max-w-md bg-card border border-border rounded-t-2xl sm:rounded-2xl flex flex-col min-h-0"
        style={{
          height: "min(88dvh, 820px)",
          paddingBottom: "env(safe-area-inset-bottom)",
        }}
      >
        <div className="shrink-0 px-4 py-3 border-b border-border bg-card rounded-t-2xl flex items-center justify-between">
          ...
          {folders.length > 0 && (
            <div className="space-y-2 pb-4">
              ...
              {folders.map((folder: any) => (
                <div key={folder.id} className="flex items-center justify-between p-3 rounded-xl border border-border bg-secondary/30 gap-3">
        ...
        <div className="shrink-0 border-t border-border bg-card px-4 pt-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)]">
          <button
